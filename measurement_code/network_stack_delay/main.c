...
void send_data_raw_socket(int fd, void *msg, int len);
...
packet_generate( void *ptr ) {
  ...
  while (1) {
    pthread_yield();
    gettimeofday(&now, NULL);    
    if(now.tv_sec - start.tv_sec > 30 ) break;
  }
  for (i = 0; i < 10; i++) {
    generate_packet(100);
  }
...
generate_packet(int len) {
  ...
  ip->daddr = inet_addr("10.3.0.12");
  
  udp = (struct udphdr *)(pkt_buf + ETHER_HDR_LEN + sizeof(struct iphdr));
  udp->source = htons(8081);
  udp->dest =  htons(8081);
  udp->len = htons(len -  ETHER_HDR_LEN);
  udp->check = 0;
  
  send_data_raw_socket(obj_cfg.intf_fd, pkt_buf, len);
}

void 
send_data_raw_socket(int fd, void *msg, int len) {
  ...
  socket_address.sll_ifindex  = obj_cfg.intf_ix;
...
initialize_raw_socket() {
  ...
  obj_cfg.intf_fd = socket(AF_PACKET,SOCK_RAW, htons(ETH_P_ALL));
  if( obj_cfg.intf_fd == -1) {
    perror("raw socket(AF_PACKET,SOCK_RAW,htons(ETH_P_ALL))");
    exit(1);
  }
  
  // bind to a specific port
  strncpy(ifr.ifr_name, obj_cfg.intf_name,IFNAMSIZ);
  if( ioctl(obj_cfg.intf_fd, SIOCGIFINDEX, &ifr)  == -1 ) {
    perror("ioctl()");
    exit(1);
  }
  obj_cfg.intf_ix = ifr.ifr_ifindex;
  ...
  if ( bind(obj_cfg.intf_fd, (struct sockaddr *) &saddrll, sizeof(struct sockaddr_ll)) == -1 ) {
    perror("bind()");
    exit(1);
  }
}
